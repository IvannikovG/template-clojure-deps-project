...
(defn widget [{fp :form-path p :path value :value on-change :on-change}]
  (let [{{y :y m :m cal :cal} :cal path :path :as data} @(rf/subscribe [:zenform/calendar fp p])]
